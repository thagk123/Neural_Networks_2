...
# Ορισμός αριθμού δειγμάτων ανά κατηγορία
"""
...
# Διαφορετικοί τιμές C, gamma και degree
C_values = [0.5, 1.0, 10.0]
gammas = [0.0005, 0.001, 0.005]
coefs = [0.0, 0.1, 0.5, 1.0]
i=0
...
for C in C_values:
    for gamma in gammas:
        for coef in coefs:
            i+=1
            print(f"\n Συνδυασμός {i}, Εκπαίδευση SVM με πυρήνα Sigmoid, C={C}, gamma={gamma} και coef0={coef}")

            # Εκπαίδευση
            start_time = time.time()
            model = SVC(kernel='sigmoid', C=C, gamma=gamma, coef0=coef)
            ...
            print(
                f"Πυρήνας: Sigmoid, C={C}, gamma={gamma} και coef0={coef}, "f"Ακρίβεια στο Training set: {train_accuracy:.2f}%, "f"Ακρίβεια στο Test set: {test_accuracy:.2f}%, "f"Χρόνος Εκπαίδευσης: {train_time/60:.2f} λεπτά, "f"Χρόνος Προβλέψεων: {pred_time/60:.2f} λεπτά")
